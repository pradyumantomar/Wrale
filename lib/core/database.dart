...
bool dayInMeasurements(DateTime d1, List<Measurement> measurements) => <bool>[
      for (final Measurement m in measurements) sameDay(d1, m.date)
...
class MeasurementDatabase {
  ...
  List<Measurement> get measurements => _measurements ??= box.values.toList()
    ..sort((Measurement a, Measurement b) => a.compareTo(b));
  ...
  List<Measurement> _calcDailyAveragedInterpolatedMeasurements() {
    ...
    for (int days = 0; days <= intervalInDays; days += 1) {
      ...
      if (!isDayInMeasurements) {
        dailyMeasurements.add(InterpolFunc.linear.function(
  ...
  List<Measurement> _calcDailyAveragedExtrapolatedMeasurements() {
    ...
    final DateTime dateStartH =
        ms.first.date.subtract(const Duration(days: _offsetInDays));
    ...
    for (int days = 1; days - 1 <= _offsetInDays; days += 1) {
      ...
      extrapolF.add(
        (InterpolFunc.linear.function(
          dateF.millisecondsSinceEpoch,
          msGaussianRegress,
        ) as Measurement)
            .apply(isMeasured: true),
      );
      extrapolH.add(
        (InterpolFunc.linear.function(
          dateH.millisecondsSinceEpoch,
          msGaussianRegress,
        ) as Measurement)
            .apply(isMeasured: true),
      );
  ...
  Measurement get lastMeasurement =>
      gaussianExtrapolatedMeasurements.lastWhere((Measurement m) =>
  ...
  double get finalSlope {
    ...
    return (mLast.weight - mCurrent.weight) /
  ...
  Duration? timeOfTargetWeight(double? targetWeight) {
    ...
    final int remainingTime = ((targetWeight - mLast.weight) / slope).round();
  ...
  Measurement? get max {
    ...
    return measurements.reduce((Measurement current, Measurement next) =>
        current.weight > next.weight ? current : next);
  }
  ...
  List<Measurement>? get longestMeasurementStrike {
    ...
    strikes.reduce((List<Measurement> current, List<Measurement> next) =>
