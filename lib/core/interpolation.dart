...
double gaussian(double x, {double sigma = 72 * 3600 * 1000, double mu = 0}) =>
    sigma == 0
        ? x - mu == 0
            ? 1
            : 0
        : 1 /
            (sigma * sqrt(2 * pi)) *
            exp(-1 / 2 * pow(x - mu, 2) / pow(sigma, 2));

/// return fermi-dirac statistic based weight with fermi-edge at 2*width
double regressionWeight(
        {required double x, required double mu, required double width}) =>
    1.0 / (1.0 + exp(((mu - x) - 2 * width) / width));
...
List<double> regressionWeights(int x, List<Measurement> measurements) {
  ...
  final double totalWeights =
      weights.reduce((double value, double element) => value + element);
  return <double>[for (final double w in weights) w / totalWeights];
}
...
List<Measurement> addLinearRegression(List<Measurement> ms) {
  ...
  msRegr.addAll(
    _createRegressionMeasurements(
      ...
      dateFrom,
      dateTo,
      interpol.extrapolationStepWidth,
    ),
  );
  return msRegr;
}
...
/// estimate eights based on measurements
...
Measurement linearInterpol(int x, List<Measurement> measurements) {
  ...
  final int index =
      measurements.lastIndexWhere((Measurement m) => m.dateInMs <= x);
  ...
  final double derivative =
      (mAfter.weight - mBefore.weight) / (mAfter.dateInMs - mBefore.dateInMs);
...
class Interpolation {
  ...
  Interpolation({
    ...
    this.extrapolationStepWidth =
        (extrapolationStepWidth * 24 * 3600 * 1000).toInt();
  }
